...
`Gtk.ApplicationFlags.DEFAULT_FLAGS` or other deprecated APIs—GTK 4 no
longer exposes these constants. GTK 4 also simplifies app startup, so
`Application.run()` is called without any arguments.
